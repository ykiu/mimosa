...
本ライブラリでは、すべての変換を行列形式で表現します。また、変換は対象要素のローカル座標で表現します。

## モジュール構成

本ライブラリは、以下の 4 つの主要なモジュールで構成されます。
...
主要なインターフェース、クラスは以下の通りです。

- **Interpreter**: Interpreter モジュールが提供するステートマシンのインターフェースです。ユーザー入力を受け取り、Motion を生成します。
- **TouchInterpreter**: タッチイベントを処理するためのクラスです。複数のタッチポイントを追跡し、ピンチやパンなどのジェスチャーを識別します。
- **MouseDragInterpreter**: マウスドラッグイベントを処理するためのクラスです。マウスの移動を追跡し、パンジェスチャーを識別します。
- **MouseWheelInterpreter**: マウスホイールイベントを処理するためのクラスです。ホイールの回転を追跡し、拡大縮小ジェスチャーを識別します。
