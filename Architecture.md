...
Store の状態遷移は reducer として記述されます。
Store の root reducer は、transform と scale の変化率のトラッキングを sub reducer ValuePrimitive に委譲します。ValuePrimitive には transform のための LinearPrimitive と、scale のための ExponentialPrimitive があります。
